...
**/
Ext.define('helloext.view.module.form.FieldSet',{
   extend:'Ext.form.FieldSet',
	 alias:'widget.formfieldset',
	 requires:[
	   'helloext.view.module.factory.FieldContainerFactory',
	   'helloext.view.module.factory.FormFieldFactory'
      ],
	 defaultType:'textfield',
	  defaults:{},
	   ...
	   config:{
		...
		schemeGroup:undefined,//定义了此fieldSet的属性以及下面需要加入的字段
		numCols:undefined,
	    formtype:undefined
      },
	  initComponent:function(){
	   this.title = this.schemeGroup.tf_formGroupName;
	   this.collapsed = this.schemeGroup.tf_collapsible;
       ...
       if(container.length>0)
		   containers.push(container);
	   //生成每一个container,一个container中可以放置若干个字段，
	   //如果分栏是3，那就放3
	   for(var i in containers){
	     this.items.push(helloext.view.module.factory.FieldContainerFactory.getContainer(containers[i],this.getViewModel(),this.formtype));
	   }
	   //加入隐藏字段
	   for(var i in hiddens){
	     var field = hiddens[i];
		 var fieldDefine = this.getViewModel().getFieldDefine(field.tf_fieldId);
		 var f = helloext.view.module.factory.FormFieldFactory.getField(fieldDefine,field,this.formtype);
		 this.items.push(f);
	   }
